...
export function getProject(store: Store, projectRoot: string): ProjectRecord | null {
  const res = store.db.exec(
    'SELECT id, project_root, name, project_type, language, description, created_at FROM projects WHERE project_root = ?',
    [projectRoot],
  );
...
export function listProjects(store: Store): ProjectRecord[] {
  const res = store.db.exec(
    'SELECT id, project_root, name, project_type, language, description, created_at FROM projects ORDER BY created_at ASC',
  );
