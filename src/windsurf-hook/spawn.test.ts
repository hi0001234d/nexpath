...
describe('spawnAuto', () => {
  ...
  it('spawns detached with stdin piped, stdout/stderr ignored', () => {
    const { spawnFn, calls } = makeSpawn();
    spawnAuto('x', { cwd: '/p', spawnFn: spawnFn as never });
    expect(calls[0].opts).toMatchObject({ cwd: '/p', detached: true, stdio: ['pipe', 'ignore', 'ignore'] });
  });
...
describe('binary resolution', () => {
  it('prefers binaryPath, then $NEXPATH_BIN, then "nexpath"', () => {
    const a = makeSpawn();
    ...
    expect(c.calls[0].bin).toBe('nexpath');
  });
});
