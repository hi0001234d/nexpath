...
export const ABSENCE_USER_PERSONA_CLARITY_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Make the user concrete enough to test against: give them a name and a context, so a design choice can be checked with 'would this user get it?' instead of 'would users in general?'", "Beyond a sketch: the user made concrete enough to test design choices against."),
...
export const ABSENCE_RETENTION_MECHANISM_CHECK_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Design the retention loop: the trigger that brings users back, the value they get each return, and how using it once compounds the next use — so it engages, not just acquires.", "Beyond a reason: the retention loop's trigger and compounding value designed."),
...
export const ABSENCE_HYPOTHESIS_BEFORE_BUILD_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Make the hypothesis falsifiable: fill every bracket — change, outcome, user, the signal that proves it, and the timeframe — and if a bracket won't fill, resolve that gap before building.", "Beyond a sketch: the hypothesis made falsifiable with a proof signal and timeframe."),
...
export const ABSENCE_TECHNICAL_VS_PRODUCT_TIME_BALANCE_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Re-anchor on direction: count recent prompts implementation-versus-product, and if skewed, step back and confirm the feature, the user, and the outcome are still right before more code.", "Beyond a sample: the product direction re-anchored on user and outcome before continuing."),
...
export const ABSENCE_NORTH_STAR_ALIGNMENT_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Test the north-star link for strength: is the path to the metric direct or two steps removed, and if there's no traceable connection, propose deferring or cutting it.", "Beyond stating a link: the north-star connection tested for strength, weak ones cut."),
...
export const ABSENCE_MANUAL_BEFORE_AUTOMATE_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Do enough rounds manually to learn the real shape: what users actually need versus what you assumed, then automate only the validated version — not the imagined one.", "Beyond one round: the manual process repeated until the validated shape is clear."),
...
export const ABSENCE_LAUNCH_STRATEGY_ABSENCE_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Build the launch plan to execute: the specific channel, the post written before launch day, the people in your network to reach, and the timing — distribution planned, not discovered.", "Beyond a sketch: the launch plan built with a written post, audience, and timing."),
...
export const ABSENCE_SOLO_MAINTAINABILITY_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Judge the full blast radius solo: if owning it when it breaks means relearning it from scratch every time, treat that ongoing cost as real and prefer the solo version you can hold in your head.", "Beyond a check: the addition judged on the solo blast radius it leaves behind."),
...
export const ABSENCE_DISTRIBUTION_THINKING_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Treat distribution as a design constraint: shape the implementation for discovery — shareable URLs, in-product sharing, referral hooks — so reaching the feature is built in, not left to magic.", "Beyond naming a path: distribution designed into the implementation, not assumed."),
...
export const ABSENCE_MONETIZATION_PATH_CLARITY_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Trace the monetization path concretely: the chain from this feature to revenue (direct, or via retention and LTV), and if the only answer is 'it's nicer', flag that as a commercial gap.", "Beyond a link: the monetization path traced to revenue, weak ones flagged."),
...
export const ABSENCE_ACCEPTANCE_CRITERIA_BEFORE_DEV_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Make the acceptance criteria independently verifiable: each in Given/When/Then or 'done when X' form, covering the primary scenario and the most likely edge case — testable by anyone.", "Beyond a draft: the acceptance criteria made independently verifiable, edge case included."),
...
export const ABSENCE_STAKEHOLDER_ALIGNMENT_CHECK_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Map every stakeholder to a concrete touchpoint — sign-off, design review, security review, eng-lead consult — and confirm each is scheduled or done before implementation, with the outcome recorded.", "Beyond a list: each stakeholder mapped to a touchpoint and its status confirmed."),
...
export const ABSENCE_REQUIREMENTS_AMBIGUITY_FLAG_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Convert every ambiguous attribute to a measurable target: name the metric, how it's measured, and the threshold that counts as met — so 'better' becomes a number anyone can check.", "Beyond flagging: each ambiguous attribute converted to a metric, method, and threshold."),
...
export const ABSENCE_DEPENDENCY_MAPPING_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Map the dependencies in both directions: every upstream blocker that could stall this, and every downstream consumer that builds on it — surfacing the integration assumptions before they surprise.", "Beyond a sketch: dependencies mapped both ways with integration assumptions surfaced."),
...
export const ABSENCE_DEFINITION_OF_DONE_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Make Done unambiguous: state the functional condition, the quality gate (tests or review), the doc requirement, and the deployment state — in 'done when X AND Y' form, so completion isn't debatable.", "Beyond a sketch: Done stated across function, quality, docs, and deploy state."),
...
export const ABSENCE_SUCCESS_METRIC_DEFINITION_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Specify the metric fully: the measure, the measurement method, the success threshold, and the timeline by which it should be met — concrete enough to evaluate post-ship without debate.", "Beyond a name: the metric specified with method, threshold, and timeline."),
...
export const ABSENCE_PRIORITY_JUSTIFICATION_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Justify the priority on evidence: the user or business value, the time-criticality, the risk it reduces, and the next-highest item deferred to make room — so priority is reasoned, not reflexive.", "Beyond one line: the priority justified on value, urgency, risk, and the deferred alternative."),
...
export const ABSENCE_USER_STORY_COMPLETENESS_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Complete the user story's who/what/why: a specific user type, the capability, and a real 'so that' outcome — and if the 'so that' won't close, resolve that before building anything.", "Beyond a draft: the user story's who/what/why completed with a closing outcome."),
...
export const ABSENCE_RISK_FLAG_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Assess each risk across categories — technical, scope, stakeholder, dependency, timeline — with a likelihood and impact rating and a mitigation-or-accept decision for each.", "Beyond a list: each risk rated and given a mitigate-or-accept decision."),
...
export const ABSENCE_SCOPE_CHANGE_IMPACT_ASSESSMENT_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Run the four-point impact assessment: what's displaced, whether the end date shifts, which downstream teams have a date dependency, and what's deferred to make room — before the change enters scope.", "Beyond a check: the scope change's four-point impact assessed before acceptance."),
...
export const ABSENCE_RETROSPECTIVE_HABIT_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    ...
    4: form("Run the retrospective with action in mind: what to preserve, what process problem to fix (without blame), and one or two specific changes to try — so the retro produces change, not just venting.", "Beyond listing: the retrospective turned into one or two actionable changes."),
