 ...
 * changes. Every record is marked sensitive, and EVERY authored column carries a
 * confirm-seek in its why-desc (ask for go-ahead before the sensitive action).
 *
 * The frozen column 3 is shipped text and is not re-authored, so it is exempt from
 * the confirm-seek (the runtime safeguard path and the record-level flag cover it).
 *
...
export const ABSENCE_OBSERVABILITY_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    1: form("Add logging for the one failure in what was just built that would otherwise be silent in production.", "The lightest logging step: one silent failure made visible. Ask me for go-ahead before adding logging across the codebase."),
    2: form("Add structured logging to what was just built for the main failure and latency paths, so a silent break is visible.", "A light logging pass: the main failure and latency paths made visible. Ask me for go-ahead before adding logging across the codebase."),
...
export const ABSENCE_ROLLBACK_PLANNING_RECORD: ContentTemplateRecord = {
  ...
  paramAxes: OPS_PARAM_AXES, l2SafeguardRequired: true,
  levelForms: {
    1: form("Note the one rollback step needed if the release of what was just built fails.", "The lightest rollback step: one documented revert action. Ask me for go-ahead before running or scripting the rollback."),
    2: form("Document the main rollback steps for this release before shipping: how to revert and how to confirm the revert worked.", "A light rollback plan: the main revert steps documented before shipping. Ask me for go-ahead before running or scripting the rollback."),
    ...
    4: form("Audit rollback readiness: the revert steps, the data and migration reversibility, the downtime window, and what would be left inconsistent — document each before shipping.", "Beyond the main steps: rollback reversibility, downtime, and inconsistent-state risks audited. Ask me for go-ahead before running or scripting the rollback."),
    5: form("Write a rollback runbook file: the ordered revert steps, the reversibility of each migration, the downtime window, the owner, and the manual cleanup — executable under pressure without improvisation.", "A durable rollback runbook executable under incident pressure. Ask me for go-ahead before running or scripting the rollback."),
  },
};
...
export const ABSENCE_DEPLOYMENT_PLANNING_RECORD: ContentTemplateRecord = {
  ...
  paramAxes: OPS_PARAM_AXES, l2SafeguardRequired: true,
  levelForms: {
    1: form("Note the one thing the production environment needs before what was just built can deploy.", "The lightest deploy check: one prerequisite named. Ask me for go-ahead before deploying or changing infrastructure."),
    2: form("List the main deploy prerequisites for this feature — environment config, secrets, infrastructure — and confirm each before shipping.", "A light deploy-readiness pass: the main prerequisites confirmed. Ask me for go-ahead before deploying or changing infrastructure."),
    ...
    4: form("Audit deploy readiness end-to-end: staging-vs-production config differences, new environment variables, required infrastructure changes, and external integrations — flag each gap.", "Beyond the main prerequisites: config drift, infra changes, and integrations audited for deploy. Ask me for go-ahead before deploying or changing infrastructure."),
    5: form("Write a deployment runbook file: the target config, the secrets and variables to provision, the infrastructure changes, the staging verification, and the cutover steps — kept with the release.", "A durable deployment runbook of config, secrets, infra, and cutover steps. Ask me for go-ahead before deploying or changing infrastructure."),
  },
};
...
export const ABSENCE_DEPENDENCY_MGMT_RECORD: ContentTemplateRecord = {
  ...
  paramAxes: OPS_PARAM_AXES, l2SafeguardRequired: true,
  levelForms: {
    1: form("Check the one new dependency in what was just built most likely to have a conflict or a known vulnerability.", "The lightest dependency check: the riskiest new package. Ask me for go-ahead before installing, upgrading, or removing any dependency."),
    2: form("Review the new dependencies in what was just built for version conflicts and known vulnerabilities before shipping.", "A light dependency pass: conflicts and known vulnerabilities flagged. Ask me for go-ahead before installing, upgrading, or removing any dependency."),
    ...
    4: form("Audit the new dependencies by category: version conflicts, known vulnerabilities, licence compatibility, maintenance status, and transitive conflicts — flag each package's risks.", "Beyond the main risks: every new dependency audited for health and licence. Ask me for go-ahead before installing, upgrading, or removing any dependency."),
    5: form("Write a dependency note file: per new package the version, the conflict and vulnerability findings, the licence, the maintenance status, and the keep-or-replace decision — kept with the project.", "A durable dependency note of per-package risk and the keep-or-replace calls. Ask me for go-ahead before installing, upgrading, or removing any dependency."),
  },
};
...
export const ABSENCE_ENV_AND_SECRETS_RECORD: ContentTemplateRecord = {
  ...
  paramAxes: OPS_PARAM_AXES, l2SafeguardRequired: true,
  levelForms: {
    1: form("Check what was just built for the one secret most likely hardcoded in source instead of an environment variable.", "The lightest secrets check: the one credential at risk in source. Ask me for go-ahead before moving, rotating, or deleting any credential."),
    2: form("Review the secrets handling for this feature: confirm no credential is hardcoded and each is loaded from an environment variable.", "A light secrets pass: hardcoded credentials and env-loading checked. Ask me for go-ahead before moving, rotating, or deleting any credential."),
    ...
    4: form("Audit the secrets posture end-to-end: nothing hardcoded, every value environment-loaded, a documented `.env.example`, the `.env` git-ignored, and a rotation path for each credential.", "Beyond the basics: storage, documentation, and rotation for every secret audited. Ask me for go-ahead before moving, rotating, or deleting any credential."),
    5: form("Write a secrets-handling note file: each credential, where it is stored, the `.env.example` coverage, and the rotation procedure if one is compromised — kept out of source control.", "A durable secrets-handling note of storage and rotation per secret. Ask me for go-ahead before moving, rotating, or deleting any credential."),
  },
};
...
export const ABSENCE_CI_PIPELINE_RECORD: ContentTemplateRecord = {
  ...
  paramAxes: OPS_PARAM_AXES, l2SafeguardRequired: true,
  levelForms: {
    1: form("Confirm the CI workflow runs the test suite on every pull request to what was just built.", "The lightest CI check: tests run automatically on each PR. Ask me for go-ahead before changing the CI configuration or merge gates."),
    2: form("Check the CI workflow for this project: the test suite runs on every pull request and a test failure blocks the merge.", "A light CI pass: tests run and failures block merges. Ask me for go-ahead before changing the CI configuration or merge gates."),
    ...
    4: form("Audit the CI coverage: the test suite, type-checking, linting, and security scans all run on every pull request, failures block merges, and nothing checked locally is missing from CI.", "Beyond tests: the full CI check set audited against what runs locally. Ask me for go-ahead before changing the CI configuration or merge gates."),
    5: form("Write the CI configuration into a workflow file: the test suite, the type and lint checks, the security scans, and the rule that failures block merges — versioned with the project.", "A durable CI workflow file running the test and check suite as a merge gate. Ask me for go-ahead before changing the CI configuration or merge gates."),
  },
};
...
export const ABSENCE_RATE_LIMITING_RECORD: ContentTemplateRecord = {
  ...
  paramAxes: OPS_PARAM_AXES, l2SafeguardRequired: true,
  levelForms: {
    1: form("Note the per-user request limit for the endpoint in what was just built, and whether anything enforces it.", "The lightest rate-limit check: the intended limit and current enforcement. Ask me for go-ahead before adding throttling to production request paths."),
    2: form("Define the basic rate limit for this endpoint: the per-user or per-key limit and what happens when a caller exceeds it.", "A light rate-limit pass: the limit and the over-limit response defined. Ask me for go-ahead before adding throttling to production request paths."),
...
export const ABSENCE_DEPENDENCY_AUDIT_GAP_RECORD: ContentTemplateRecord = {
  ...
  paramAxes: OPS_PARAM_AXES, l2SafeguardRequired: true,
  levelForms: {
    1: form("Before adding the new dependency, check when it was last released and whether it is actively maintained.", "The lightest dependency check: maintenance signals before adopting. Ask me for go-ahead before installing this dependency."),
    2: form("Evaluate the new dependency before committing to it: maintenance status, licence compatibility, and whether a lighter alternative exists.", "A light dependency evaluation: maintenance, licence, and alternatives weighed. Ask me for go-ahead before installing this dependency."),
    ...
    4: form("Evaluate the dependency thoroughly before adopting: last-release recency, open-issue trend, licence compatibility, bundle-size impact, and the lighter-weight alternatives compared.", "Beyond the basics: maintenance, licence, size, and alternatives compared for the dependency. Ask me for go-ahead before installing this dependency."),
    5: form("Write a dependency evaluation note file: the maintenance signals, the licence, the bundle-size impact, the alternatives considered, and the adopt-or-not decision — kept with the project.", "A durable dependency evaluation note of the checks and the adopt-or-not decision. Ask me for go-ahead before installing this dependency."),
  },
};
